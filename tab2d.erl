    ...
    %------------------------------------------------- Join & Split Array for 2 nodes
     
        split2(Array) ->
                S = tab2d:sizeH(Array),
                Step = trunc(S/2),
                D1 = tab2d:create(S,Step+1),
                D2 = tab2d:create(S,Step+1),
                split2(Array,S,Step,D1,D2).
         
        split2(Array,S,St1,D1,D2) ->
                split2X(Array,S,St1,D1,D2,0).
               
        split2X(Array,S,St1,D1,D2,IX) when IX < S ->
                Tmp1 = split2Y(Array,S,0,St1+1,D1,IX,0),
                Tmp2 = split2Y(Array,S,St1-1,S,D2,IX,0),
                split2X(Array,S,St1,Tmp1,Tmp2,IX+1);
               
        split2X(_,S,_,D1,D2,S) -> {D1,D2}.
               
        split2Y(Array,S,IY,K,D,IX,C) when IY < K ->
                El = tab2d:get(IX,IY,Array),
                Nowa = tab2d:set(IX,C,El,D),
                split2Y(Array,S,IY+1,K,Nowa,IX,C+1);
         
        split2Y(_,_,K,K,D,_,_) -> D.
        ...
        join2(A1,A2) ->
                        ...
                        Step = trunc(HS/2),
                        join2X(New,A1,A2,HS,Step,0).
                   
        join2X(Dest,A1,A2,HS,Step,IX) when IX < HS ->
                        W1 = join4Y(Dest,A1,0,IX,0,Step+1),
                        W2 = join4Y(W1,A2,Step,IX,1,Step+1),
                        join2X(W2,A1,A2,HS,Step,IX+1);
         
        join2X(Dest,_,_,HS,_,HS) -> Dest.
                                   
        join2Y(Dest,Src,Pocz,IX,IY,WS) when IY < WS ->
                        El = tab2d:get(IX,IY,Src),
                        Nowa = tab2d:set(IX,Pocz,El,Dest),
                        join2Y(Nowa,Src,Pocz+1,IX,IY+1,WS);
         
        join2Y(Dest,_,_,_,WS,WS) -> Dest.
